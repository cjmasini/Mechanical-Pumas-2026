...
public class VisionSubsystem extends SubsystemBase {
    ...
    public void periodic() {
        ...
        SmartDashboard.putBoolean("Vision/HasRecentVision", hasRecentVision());
        SmartDashboard.putNumber("Vision/SecondsSinceLastVision", getSecondsSinceLastVision());

        // Publish targeting info
        Pose2d robotPose = driveSubsystem.getPose();
        SmartDashboard.putNumber("Vision/DistanceToHub", getDistanceToHub());
        SmartDashboard.putNumber("Vision/AngleToHub", getAngleToHub().getDegrees());
        SmartDashboard.putNumber("Vision/RobotX", robotPose.getX());
        SmartDashboard.putNumber("Vision/RobotY", robotPose.getY());
        SmartDashboard.putNumber("Vision/RobotHeading", robotPose.getRotation().getDegrees());
        SmartDashboard.putNumberArray("Vision/RobotPose",
            new double[] { robotPose.getX(), robotPose.getY(), robotPose.getRotation().getRadians() });
    }
    ...
    /**
     * Checks if we have received a vision measurement recently.
     *
     * @return true if vision data is fresh (within threshold)
     */
    ...
    /**
     * Gets the time since the last valid vision measurement.
     *
     * @return Seconds since last vision update
     */
    public double getSecondsSinceLastVision() {
